...
def conectar_planilha():
    try:
        ...
        st.error(f"Erro ao conectar com o Google Sheets. Verifique os Segredos. Detalhe: {e}")
        st.stop()
...
def carregar_dados():
    ...
    if dados_iqs.empty or dados_tecnicos.empty:
        st.error("A planilha do Google está vazia ou sem cabeçalhos.")
        st.stop()
...
def salvar_nova_senha(re_usuario, nova_senha):
    ...
    for idx, row in enumerate(registros):
        if str(row.get('re_iq', '')).strip().replace('.0','') == str(re_usuario):
            ws.update_cell(idx + 2, list(row.keys()).index('senha') + 1, nova_senha)
            break
    st.cache_data.clear() 
...
def atualizar_acompanhamento(login_tecnico, status, contrato="", data_mon="", obs=""):
    ...
    for idx, row in enumerate(registros):
        if str(row.get('login', '')).strip().replace('.0','') == str(login_tecnico):
            if 'Acompanhamento' in row: ws.update_cell(idx + 2, list(row.keys()).index('Acompanhamento') + 1, status)
...
if not st.session_state['logado']:
    ...
    with tab_senha:
        st.info("Confirme seu RE e sua senha atual para criar uma nova.")
        ...
        if st.button("Salvar Nova Senha"):
            ...
            if not iq_valido.empty:
                ...
                st.success("Senha alterada no Google Sheets! Pode fazer login.")
            else:
                st.error("RE ou Senha atual incorretos.")
    ...
    if st.session_state['pagina_atual'] == "Dashboard":
            ...
            for tec_nome in tecnicos_selecionados:
                ...
                with st.expander(f"👤 {tec_nome}"):
                    ...
                    if st.button("Salvar Dados no Banco de Dados", key=f"btn_{tec_login}", type="primary"):
                        ...
                        st.success("Dados salvos e status alterado para SIM na planilha!")

    elif st.session_state['pagina_atual'] == "Matinal":
        st.title("📅 Agendamento e Realização da Matinal")
        tab_agenda, tab_exec = st.tabs(["1. Agendar Matinal", "2. Executar Vistoria"])
        ...
        with tab_agenda:
            ...
            if st.button("Adicionar à Agenda", type="primary"):
                if tec_agendar != "Selecione..." and data_agendada is not None:
                    ...
                    st.warning("Selecione o técnico e preencha uma data.")
            
            st.divider()
            st.write("**Sua Agenda Atual:**")
            if not st.session_state['agenda_matinal']:
                st.info("Nenhuma matinal agendada no momento.")
            else:
                # LISTA COM BOTÃO DE APAGAR
                for tec, data in list(st.session_state['agenda_matinal'].items()):
                    col_info, col_del = st.columns([4, 1])
                    col_info.write(f"📌 **{data}** - Técnico: {tec}")
                    if col_del.button("🗑️ Apagar", key=f"del_{tec}"):
        ...
        with tab_exec:
            ...
            if tec_atual != "Selecione...":
                st.info("⚠️ Assinale **apenas o que estiver faltando ou irregular**.")
                faltas = []
                
                t1, t2, t3, t4, t5 = st.tabs(["Manuais", "Acessórios", "GPON/Fibra", "EPI/Segurança", "Veículo/Asseio"])
                
                with t1:
                    if st.checkbox("Alicates (Crimpador, Bico, Corte, Universal)"): faltas.append("Alicates")
                    if st.checkbox("Chaves de Fenda / Phillips"): faltas.append("Chaves")
                    if st.checkbox("Chave Trava Lock / GTP"): faltas.append("Chaves de segurança")
                ...
                with t2:
                    ...
                    if st.checkbox("Furadeira e Brocas"): faltas.append("Furadeira/Brocas")
                ...
                with t3:
                    if st.checkbox("Clivador / Gabaritos"): faltas.append("Clivador/Gabarito")
                    if st.checkbox("Alicate Decapador Fibra/Drop"): faltas.append("Alicate Óptico")
                    if st.checkbox("Medidores (DBAM/Power Meter)"): faltas.append("Medidores Ópticos")
                    if st.checkbox("Álcool Isopropílico / Lenços"): faltas.append("Kit Limpeza Fibra")
                with t4:
                ...
                with t5:
                    ...
                    if st.checkbox("Veículo (Sujo, Desorganizado ou Avarias)"): faltas.append("Problemas no Veículo")
                    if st.checkbox("PDA (Deslogado ou Bateria < 50%)"): faltas.append("PDA Irregular")

                st.divider()
                foto_upload = st.file_uploader("📸 Enviar Foto", type=['png', 'jpg'])
                ...
                if st.button("Concluir e Enviar E-mail", type="primary"):
                    if not foto_upload:
                        ...
                        st.success(f"Matinal concluída e gravada na planilha Google!")
                        # Botão HTML corrigido para enviar o email sem erros visuais
                        st.markdown(f'<a href="{url_email}" target="_blank" style="display: inline-block; padding: 0.5em 1em; color: white; background-color: #007BFF; border-radius: 4px; text-decoration: none;">📩 Clique aqui para enviar o relatório por E-mail</a>', unsafe_allow_html=True)
